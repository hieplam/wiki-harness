...
### Task 1: T26 — cut the v1.0.0 release

Requirement contract: `/Users/hip/repo/wiki-harness-analysis/plan/briefs/T26.md` — read it in full, including any trailing section headed
"Warchief amendment". Work on branch `task/T26-cut-v1.0.0`, following the branching convention in the
campaign constraints file.

Shaman amendment for this campaign, binding and additional to the brief: BEFORE tagging, delete the two generated campaign-doc directories `docs/specs/` and `docs/plans/` from the repository in this same task, so the tagged release carries no campaign bookkeeping. The owner ruled this on 2026-08-26. The real contract documents are unaffected: they live outside this repository, in `/Users/hip/repo/wiki-harness-analysis/plan/`. Delete them in the release commit itself, then tag. Cut the branch from `main`, and tick this task's own `docs/PLAN.md` box in the same commit.

Acceptance commands, run verbatim in a fresh shell:

```sh
cd /Users/hip/repo/wiki-harness && python3 -m unittest discover -s tests -q
cd /Users/hip/repo/wiki-harness && git tag -l v1.0.0
```
...
- [ ] **Step 1: Commit** — one or more conventional commits, each ending with the three
      trailers from the campaign constraints file, using `Tribe-Task: 35/40`, and
