...
### Task 1: T16 — upgrade.py step 1: refuse-before-write drift check including missing-path drift

Requirement contract: `/Users/hip/repo/wiki-harness-analysis/plan/briefs/T16.md` — read it in full, including any trailing section headed
"Warchief amendment". Work on branch `task/T16-drift-check`, following the branching convention in the
...
cd /Users/hip/repo/wiki-harness && python3 -m unittest tests.test_upgrade -v
```
...
- [ ] **Step 1: Commit** — one or more conventional commits, each ending with the three
      trailers from the campaign constraints file, using `Tribe-Task: 23/40`, and
