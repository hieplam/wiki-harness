...
### Task 1: T23 — The commit flag plus automatic rollback on a failed post-write self-check

Requirement contract: `/Users/hip/repo/wiki-harness-analysis/plan/briefs/T23.md` — read it in full, including any trailing section headed
"Warchief amendment". Work on branch `task/T23-commit-autorollback`, following the branching convention in the
...
- [ ] **Step 1: Commit** — one or more conventional commits, each ending with the three
      trailers from the campaign constraints file, using `Tribe-Task: 31/40`, and
