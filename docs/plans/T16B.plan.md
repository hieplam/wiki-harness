...
### Task 1: T16B — Core apply pipeline: fetch target, scratch-copy, overwrite, lint scratch, bare promote-copy, write manifest

Requirement contract: `/Users/hip/repo/wiki-harness-analysis/plan/briefs/T16B.md` — read it in full, including any trailing section headed
"Warchief amendment". Work on branch `task/T16B-core-apply-pipeline`, following the branching convention in the
...
cd /Users/hip/repo/wiki-harness && python3 -m unittest tests.test_upgrade.TestApplyPipeline -v
```
...
- [ ] **Step 1: Commit** — one or more conventional commits, each ending with the three
      trailers from the campaign constraints file, using `Tribe-Task: 24/40`, and
